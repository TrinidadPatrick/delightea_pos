import { View, Text, ScrollView, TouchableOpacity, ToastAndroid, Alert, Dimensions } from 'react-native'
...
useFocusEffect(
  ...
  const handleDestructure = () => {
    return Categories?.sort((a,b)=> new Date(a.created_at) - new Date(b.created_at)).filter((category_t)=> category_t.category_name !== 'All').map((category) => ({
            ...
            <View className={` ${category?.products?.length <= 0 && 'hidden'}  flex flex-col justify-start items-start`} key={index}>
              {
                ...
                <Text className="text-center text-2xl mb-1 font-bold text-green-500">{category?.category_name}</Text>
                <View className="w-full flex-1">
                  {
                    category?.products?.length !== 0 && category?.products?.sort((a, b) => a.product_name.localeCompare(b.product_name)).map((product, index) => {
                      return (
